    ...
    var handler =  function () {
        ...
        try {
            var CB = {
                ...
                list: [],
                start: 0,
                size: config.fileManagerListSize,
                total: 0,
                error: null
            };
            var start =req.query.start? Number("0" + req.query.start) :0;
            var size = req.query.size?  Number("0" + req.query.size) : config.fileManagerListSize;
            var allFileList = [];
            var path = req.app.get("RootPath") + config.fileManagerListPath;
            ...
            then(function (defer) {
                ...
                files.forEach(function (item) {
                    ...
                    if(ext && ext!==''){
                        for(var i=0;i<config.fileManagerAllowFiles.length;i++){
                            if(ext.toLowerCase()==config.fileManagerAllowFiles[i]){
