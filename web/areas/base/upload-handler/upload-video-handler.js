...
module.exports = function (req, res) {
    ...
    var handler = async function () {
        ...
        var form = new multiparty.Form({
            maxFilesSize: config.videoMaxSize,
        ...
        form.parse(req,async function(err,fields,files){
            try{
                if(err){
                    ...
                    var file = files.upfile[config.videoFieldName][0];
                    ...
                    if(file){
                        ...
                        if(Number(file.size)>config.videoMaxSize){
                            CB.state=GetStateMessage(UploadState.SizeLimitExceed);
                            res.json(CB);
                            return;
                        }
                        ...
                        var PathFormat=config.videoPathFormat;
    ...
    var CheckFileType=function(ext){
       for(var i=0;i< config.videoAllowFiles.length ;i++){
           if(config.videoAllowFiles[i]==ext.toLowerCase()){
               return true;
           }
       }
       return false;
